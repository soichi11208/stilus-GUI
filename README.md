...
C++20 対応コンパイラと CMake 3.20 以上が必要。`pkg-config` で検出するような外部システムライブラリは不要 — XCB は `third_party/xcb` にベンダリングして静的リンク済み。システムから取得するのは XCB が推移的に必要とする `libXau`/`libXdmcp` (X11 認証) のみ。
...
```sh
cmake --build build
ctest --test-dir build --output-on-failure   # 単体テスト + ゴールデンイメージテスト
